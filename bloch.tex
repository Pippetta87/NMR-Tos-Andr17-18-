...
\section{Tempi di rilassamento}\linkdest{relaxation}
...
\begin{figure}[!ht]\includegraphics[trim={0cm 0cm 0 0},clip, keepaspectratio,width=0.9\textwidth]{RFcycle}\label{fig:RFcycle}\end{figure}
\end{block}
\end{column}
\end{columns}
\end{frame}
...
\begin{align*}
&\TDof{t}\rho_{\alpha}=-W_-\rho{\alpha}+W_+\rho_{\beta}\\
&\TDof{t}\rho_{\beta}=W_-\rho{\alpha}-W_+\rho_{\beta}\\
&M_z\propto(\rho_{\alpha}-\rho_{\beta})
\end{align*}
\end{block}
\end{column}
...
\begin{frame}{DD relaxation (L584)}
...
\begin{figure}[!ht]
    \centering
    \includegraphics[keepaspectratio,width=0.6\textwidth]{sdztransP}
    \label{fig:sdztransP}
\end{figure}
